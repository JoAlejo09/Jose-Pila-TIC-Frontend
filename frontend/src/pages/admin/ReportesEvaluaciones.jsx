...
const ReportesEvaluaciones = () => {
  ...
  const handleEliminar = async (id) => {
    const confirmar = window.confirm("Desea eliminar el resultado?");
  ...
  const materiasUnicas = useMemo(() => {
    return [
      ...new Set(
        resultados.map((resultado) => resultado.cuestionario?.materia?.nombre),
      ),
    ].filter(Boolean);
  }, [resultados]);
  ...
  const temasUnicos = useMemo(() => {
    return [
      ...new Set(
        resultados.map((resultado) => resultado.cuestionario?.tema?.nombre),
      ),
    ].filter(Boolean);
  }, [resultados]);
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <input
            ...
            className="border border-gray-300 rounded-xl p-3 focus:outline-none
            ...
            <tbody>
              {resultados.length === 0 ? (
                ...
                resultados.map((resultado) => (
                  <tr key={resultado._id} className="border-t hover:bg-gray-50">
                    {/* ESTUDIANTE */}
                    <td className="p-4">
                      <div>
                        <p className="font-semibold text-gray-800">
                          {resultado.estudiante?.nombre || "Sin nombre"}{" "}
                          {resultado.estudiante?.apellido || ""}
                        </p>

                        <p className="text-sm text-gray-500">
                          {resultado.estudiante?.email || "Sin email"}
                        </p>
                      </div>
                    </td>
                    ...
                    {/* TEMA */}
                    <td className="p-4 text-gray-700">
                      {resultado.cuestionario?.tema?.nombre || "General"}
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => handleEliminar(resultado._id)}
