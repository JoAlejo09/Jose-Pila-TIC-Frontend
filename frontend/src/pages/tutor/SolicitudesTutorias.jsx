...
const SolicitudesTutorias = () => {
  ...
  const cargarTutorias = async () => {
    try {
      ...
      const data = await obtenerTutoriasPendientesRequest();
  ...
  const aceptarTutoria = async (id) => {
    try {
      ...
      alert(error.response?.data?.msg || "Error al aceptar tutoría");
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold">Solicitudes de Tutorías</h1>

          <p className="text-gray-500">Tutorías pendientes disponibles</p>
        </div>
      </div>
          ...
          {tutorias.map((tutoria) => (
            <div key={tutoria._id} className="bg-white shadow rounded-xl p-6">
              <div className="flex justify-between items-start">
                ...
                <button
                  onClick={() => aceptarTutoria(tutoria._id)}
