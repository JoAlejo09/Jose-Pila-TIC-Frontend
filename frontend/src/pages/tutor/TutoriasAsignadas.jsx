...
const TutoriasAsignadas = () => {
  ...
  const cargarTutorias = async () => {
    try {
      ...
      const data = await obtenerTutoriasTutorRequest();
  ...
  const marcarRealizada = async (id) => {
    const confirmar = window.confirm("¿Marcar tutoría como realizada?");
    ...
    try {
      ...
      alert(error.response?.data?.msg || "Error al actualizar tutoría");
    }
  };

  // LIBERAR TUTORIA
  const liberarTutoria = async (id) => {
    const confirmar = window.confirm("¿Desea liberar esta tutoría?");
    ...
    try {
      ...
      alert(error.response?.data?.msg || "Error al liberar tutoría");
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-500">Tutorías aceptadas y realizadas</p>
      </div>
      ...
      {tutorias.length === 0 ? (
        ...
        <div className="grid gap-5">
          {tutorias.map((tutoria) => (
            <div key={tutoria._id} className="bg-white shadow rounded-xl p-6">
              <div className="flex justify-between items-start gap-4">
                <div className="space-y-2 flex-1">
                  <div className="flex items-center gap-3">
                    <h2 className="text-xl font-bold">{tutoria.materia}</h2>
                  ...
                  <p>
                    ...
                    {tutoria.estudiante?.nombre} {tutoria.estudiante?.apellido}
                  </p>
                  ...
                  {tutoria.descripcion && (
                    <div>
                      ...
                      <p className="text-gray-600">{tutoria.descripcion}</p>
                    </div>
                  )}
                  ...
                  {tutoria.calificacion && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-3">
                      <p className="font-semibold">Calificación recibida</p>
                      <p className="mt-1">⭐ {tutoria.calificacion}/5</p>
                ...
                {tutoria.estado === "aceptada" && (
                  <div className="flex flex-col gap-2">
                    <button
                      onClick={() => marcarRealizada(tutoria._id)}
                    ...
                    <button
                      onClick={() => liberarTutoria(tutoria._id)}
