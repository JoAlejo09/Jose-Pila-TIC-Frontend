...
import { obtenerUnidadesRequest } from "../../services/unidadService.js";

const ModalTema = ({ onClose, onTemaCreado, modo = "crear", temaSeleccionado }) => {
  ...
  useEffect(() => {
    const cargarUnidades = async () => {
      try {
        ...
        const activas = data.filter((u) => u.estado);
        setUnidades(activas);
      } catch (error) {
        console.log(error);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modo === "crear") {
        ...
        await actualizarTemaRequest(temaSeleccionado._id, form);
      }
      ...
      console.log(error);
      setError(error.response?.data?.msg || "Error al guardar tema");
    }
  };
    ...
    >
      ...
      >
        ...
        <h2 className=" text-2xl font-bold mb-6 text-center ">
          {modo === "crear" ? "Nuevo Tema" : "Editar Tema"}
        </h2>
