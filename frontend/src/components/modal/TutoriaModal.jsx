...
import { crearTutoriaRequest, editarTutoriaRequest} from "../../services/tutoriaService.js";

const TutoriaModal = ({ onClose, recargarTutorias, modo = "crear", tutoria = null}) => {
  const [form, setForm] = useState({
    ...
    fecha: tutoria?.fecha
      ? new Date(tutoria.fecha).toISOString().slice(0, 16)
      : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let data;
      if (modo === "editar") {
        data = await editarTutoriaRequest(tutoria._id, form);
      } else {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      ...
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-xl p-8 relative max-h-[90vh] overflow-y-auto">
        ...
        <h1 className="text-3xl font-bold mb-2">
          {modo === "editar" ? "Editar Tutoría" : "Solicitar Tutoría"}
        </h1>
